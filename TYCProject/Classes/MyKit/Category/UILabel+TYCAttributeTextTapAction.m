//
//  UILabel+TYCAttributeTextTapAction.m
//  OnlineHosptial
//
...
@implementation UILabel (TYCAttributeTextTapAction)
...
- (id<TYCAttributeTapActionDelegate>)delegate
{
    return objc_getAssociatedObject(self, _cmd);
}
...
- (void)setDelegate:(id<TYCAttributeTapActionDelegate>)delegate
{
    objc_setAssociatedObject(self, @selector(delegate), delegate, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

#pragma mark - mainFunction
- (void)tyc_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings tapClicked:(void (^) (NSString *string , NSRange range , NSInteger index))tapClick
{
    [self tyc_getRangesWithStrings:strings];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    [self tyc_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        
        if (weakSelf.tapBlock) {
            weakSelf.tapBlock (string , range , index);
        }
        
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(tyc_attributeTapReturnString:range:index:)]) {
            [weakSelf.delegate tyc_attributeTapReturnString:string range:range index:index];
        }
        
        if (self.isTapEffect) {
            
            [self tyc_saveEffectDicWithRange:range];
            
            [self tyc_tapEffectWithStatus:YES];
        }
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (self.isTapAction) {
        if ([self tyc_getTapFrameWithTouchPoint:point result:nil]) {
            return self;
        }
    }
...
- (BOOL)tyc_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock
{
    ...
    CGAffineTransform transform = [self tyc_transformForCoreText];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGRect flippedRect = [self tyc_getLineBounds:line point:linePoint];
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; j++) {
                ...
                TYCAttributeModel *model = self.attributeStrings[j];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    if (self.isTapEffect) {
        ...
        [self performSelectorOnMainThread:@selector(tyc_tapEffectWithStatus:) withObject:nil waitUntilDone:NO];
...
- (CGAffineTransform)tyc_transformForCoreText
{
    return CGAffineTransformScale(CGAffineTransformMakeTranslation(0, self.bounds.size.height), 1.f, -1.f);
}

- (CGRect)tyc_getLineBounds:(CTLineRef)line point:(CGPoint)point
{
    CGFloat ascent = 0.0f;
...
- (void)tyc_tapEffectWithStatus:(BOOL)status
{
    if (self.isTapEffect) {
...
- (void)tyc_saveEffectDicWithRange:(NSRange)range
{
...
- (void)tyc_getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            totalStr = [totalStr stringByReplacingCharactersInRange:range withString:[weakSelf tyc_getStringWithRange:range]];
            
            TYCAttributeModel *model = [TYCAttributeModel new];
...
- (NSString *)tyc_getStringWithRange:(NSRange)range
{
...
@implementation TYCAttributeModel
