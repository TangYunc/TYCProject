...
- (void)viewDidLoad {
    ...
    if (!tzImagePickerVc.sortAscendingByModificationDate && _isFirstAppear && iOS8Later) {
        [[TZImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(TZAlbumModel *model) {
            self->_model = model;
            self->_models = [NSMutableArray arrayWithArray:_model.models];
            [self initSubviews];
        }];
    } else {
...
- (void)configBottomToolBar {
    ...
    UIView *bottomToolBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.tz_height - 50 - kSafeAreaHeight, self.view.tz_width, 50)];
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        _originalPhotoButton.frame = CGRectMake(CGRectGetMaxX(_previewButton.frame), self.view.tz_height - 50 - kSafeAreaHeight, fullImageWidth + 56, 50);
